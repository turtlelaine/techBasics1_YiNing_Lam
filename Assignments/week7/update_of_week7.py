...
#new update for week 7 asm: line 21-70, 722-729, 751-753
...
def load_records():
    try:
        ...
        print("Error: Record file corrupted. Creating new one.")
        return []
...
def ending_count():
    ...
    for ending in endings:
        if ending in ending_conditions:
            count += 1
    return count
...
def main():
    ...
    if DEBUG:
        save_record(player_name, 5, "5/5 endings unlocked (DEBUG)")
        show_leaderboard()
        return
    ...
    unlocked_count = ending_count()
    ending_message = f"{unlocked_count}/5 endings unlocked"
    save_record(player_name, unlocked_count, ending_message)
