...
def save_game(notebook, repeat, loop, sunset_choice=None):
    save_data = {
        "notebook": notebook,
        "repeat": repeat,
        "loop": loop,
        "sunset_choice": sunset_choice
    }
...
def load_game():
    ...
    try:
        with open(SAVE_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        print("\n Record loaded.")
        return data["notebook"], data["repeat"], data["loop"], data.get("sunset_choice")
    except Exception as e:
        print(f"\n File loading failed. {e}")
        return None
...
def save_endings(endings):
    with open(ENDINGS_FILE, "w", encoding="utf-8") as f:
        json.dump(endings, f, indent=2)

def record_ending(ending_name, endings_collected):
    if ending_name not in endings_collected:
        endings_collected.append(ending_name)
        save_endings(endings_collected)
        print(f"New ending unlocked：{ending_name} ")
        print(f"Unlocked endings：{len(endings_collected)} / 5")
        time.sleep(2)
    return endings_collected

def show_endings_progress(endings_collected):
    endings_list = ["Exhaustion", "I'm coming for you", "Awaken Dream", "Unchanged Daily Life", "See You Again"]
    print("\n Endings: ")
    for ending in endings_list:
        if ending in endings_collected:
            print(f"  [✓] {ending}")
        else:
            print(f"  [ ] {ending}")
    print()
...
def game_intro():
    ...
    time.sleep(1)
    print("=")
    print("\nAn old notebook lies quietly on your bedside.")
    print("You vaguely feel that it contains some important guidance...")
    time.sleep(1.5)

    if os.path.exists(SAVE_FILE):
        print("New return detected!")
        choice = input("Continue last game？(yes/no): ")
        if choice.lower() == "yes":
            loaded = load_game()
            if loaded:
                return "load", loaded
    ...
    while True:
        start = input('\nPlease enter "Start" to open the notebook: ')
        if start == "Start":
...
def step1(notebook, repeat, loop, endings_collected):
    print("\n[ Step 1: Falling Asleep ]")
    print("===")
    time.sleep(1.5)

    print(
        "You've been suffering from insomnia for a long time. Your doctor recommended stopping use of phone one hour before sleep.")
    time.sleep(1.5)
    ...
    while phone_loop:
        ...
        if choice == "yes":
            ...
            time.sleep(1)
            print(
                "You saw a photo of the two of you on the screen, memories flooding in while tears run down your face.")
            time.sleep(1.5)
            ...
            if repeat >= 3:
                ...
                endings_collected = record_ending("Exhaustion", endings_collected)
                show_endings_progress(endings_collected)

                retry = input(
                    '\nDo you want to wake up and try again? (Enter "retry" to restart / Enter "end" to quit): ')
                if retry == "retry":
                    print("You closed your eyes again...")
                    time.sleep(1.5)
                    delete_save()
                    return "restart", 0, 0, [], endings_collected
                else:
    ...
    return "continue", repeat, loop, notebook, endings_collected

def step2(loop, notebook):
    print("===")
...
def step5(loop, notebook, repeat, endings_collected):
    print("===")
    print("[ Step 5: Reset ]")
    print("===")
    time.sleep(1.5)
    ...
    if loop >= 3:
        ...
        endings_collected = record_ending("I'm coming for you", endings_collected)
        show_endings_progress(endings_collected)
    ...
    if record == "":
        ...
        time.sleep(2)
        return "reset_loop", loop, repeat, notebook, endings_collected
    else:
        print("Your record doesn't match the dream. The reset is experiencing anomalies...")
        time.sleep(1.5)

    return "continue", loop, repeat, notebook, endings_collected


def steps_6_9(sunset, notebook, loop, repeat, endings_collected):
    print("===")
    ...
    try:
        ...
        if chosenNumber < len(memoryRecord):
            ...
            print(f"\nYou close your eyes and recall: {selectedNumber}")
            distance_map = {
                "sunset": "very close, almost within reach",
                "phone": "a bit far, but you can see his silhouette",
                "dream": "sometimes near, sometimes far, like through a fog",
                "approach": "right in front of you, within arm's reach",
                "stay": "neither far nor near, but you see him clearly",
                "noname": "very close, but his face is blurry"
            }
            distance = distance_map.get(memoryInput, "you feel he's not far away")
            print(f"You feel he is {distance}.")
            notebook.append(f"I recalled: {selectedNumber}. He was {distance} from me.")

        else:
            print("\n" + "=" * 50)
            print("You chose blank. You can't remember anything.")
            time.sleep(1)
            print("He is far, far away, like a whole world apart.")
            time.sleep(1)
            print("You try to chase him, but the dream starts crumbling around you...")
            time.sleep(1.5)
            print("You're neither fully asleep nor fully awake.")
            print("Trapped in between, you can't move forward or backward.")
            print("[Bad Ending: Awaken Dream]")
            print("=" * 50)

            endings_collected = record_ending("Awaken Dream", endings_collected)
            show_endings_progress(endings_collected)

            retry = input('\nDo you want to try again? (retry/end): ')
            if retry == "retry":
                print("You closed your eyes again...")
                time.sleep(1.5)
                print("The dream is reweaving...")
                time.sleep(1.5)
                delete_save()
                return "restart", endings_collected
            else:
                print("The notebook quietly closes, waiting to be opened again...")
                exit()
    except:
        print("You hesitated too long. The memory begins to blur...")
        time.sleep(1)
        print("The dream starts fading away.")
        time.sleep(1)
        print("You're left in a gray, empty space.")
        time.sleep(1.5)
        print("Not awake. Not asleep. Just... existing.")
        print("[Bad Ending: Awaken Dream]")
        endings_collected = record_ending("Awaken Dream", endings_collected)
        show_endings_progress(endings_collected)
...
def step10(endings_collected):
    ...
    if daily != "no":
        ...
        endings_collected = record_ending("Unchanging Daily Life", endings_collected)
        show_endings_progress(endings_collected)

        return "normal_end", endings_collected

    return "true_ending_path", endings_collected

def true_ending(endings_collected):
    ...
    if reset != "":
        ...
        time.sleep(1.5)
        return "restart_game", endings_collected
    ...
    time.sleep(2)
    print("After a strong fluctuation, you entered a void.")
    time.sleep(1.5)
    ...
    time.sleep(1.5)
    print(
        "You're a little annoyed, complaining to yourself that you think of him so often, yet he hasn't visited your dreams for even once.")
    time.sleep(2)
    ...
    time.sleep(2)
    print(
        "If you enjoyed this story, here's the adapted short film directed by me: https://youtu.be/Sh820Sh9ffQ?si=6FQyPpZEB1UWHIoN")
    print("I hope you enjoyed them as much as I did :D")
    print("See ya")

    endings_collected = record_ending("See You Again", endings_collected)
    show_endings_progress(endings_collected)

    return "game_end", endings_collected

def main():
    endings_collected = load_endings()
    intro_result, intro_data = game_intro()

    if intro_result == "load":
        notebook, repeat, loop, sunset_choice = intro_data
        print(f"\nDream load, time record: {intro_result}")
        time.sleep(1)
    else:
        notebook = []
        repeat = 0
    ...
    while True:
        result, new_repeat, new_loop, new_notebook, endings_collected = step1(notebook, repeat, loop, endings_collected)
        if result == "restart":
            notebook = []
            repeat = 0
            loop = 0
        ...
        save_game(notebook, repeat, loop, sunset_choice)

        loop += 1
        sunset_choice, notebook = step2(loop, notebook)
        notebook = step3(notebook)
        step4(loop)

        save_game(notebook, repeat, loop, sunset_choice)

        reset_result, new_loop, new_repeat, new_notebook, endings_collected = step5(loop, notebook, repeat,
                                                                                    endings_collected)
        if reset_result == "restart":
            notebook = []
            repeat = 0
            loop = 0
            continue
        elif reset_result == "reset_loop":
            loop = new_loop
            repeat = new_repeat
            notebook = new_notebook
            continue

        endings_collected = steps_6_9(sunset_choice, notebook, loop, repeat, endings_collected)
        save_game(notebook, repeat, loop, sunset_choice)
        awakening_result, endings_collected = step10(endings_collected)
        if awakening_result == "normal_end":
            delete_save()
            break

        true_ending_result, endings_collected = true_ending(endings_collected)
        if true_ending_result == "restart_game":
            notebook = []
            repeat = 0
            loop = 0
            continue
